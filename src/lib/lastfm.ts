...
export async function searchTracks(query: string): Promise<Track[]> {
  const res = await fetchWithProxy(
    `${BASE_URL}?method=track.search&track=${encodeURIComponent(
      query
  ...
  return Promise.all(
    tracks.map(async (t: any, i: number) => {
      try {
        const infoRes = await fetchWithProxy(
          `${BASE_URL}?method=track.getInfo&artist=${encodeURIComponent(
            t.artist
        ...
        if (
          ...
          imageUrl =
            (await getItunesArtwork(t.artist, t.name)) ||
...
export async function getSimilarTracks(
  ...
  const res = await fetchWithProxy(
    `${BASE_URL}?method=track.getsimilar&artist=${encodeURIComponent(
      artist
    )}&track=${encodeURIComponent(
      track
    )}&api_key=${API_KEY}&format=json&limit=20`
    ...
    tracks.map(async (t: any, i: number) => {
      try {
        ...
        if (
          ...
          imageUrl =
            (await getItunesArtwork(t.artist.name, t.name)) ||
            `https://picsum.photos/seed/${i}/48`;
        }

        const { bpm, key } = await getBpmAndKey(
          t.artist.name,
          t.name
        );
